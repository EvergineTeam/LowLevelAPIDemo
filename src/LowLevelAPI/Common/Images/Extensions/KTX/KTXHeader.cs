...
namespace Evergine.Assets.Extensions.KTX
{
    ...
    {
        /// <summary>
        /// Endianness contains the number 0x04030201 written as a 32-bit integer. If the file is little-endian, this is represented as the bytes 0x01 0x02 0x03 0x04.
        /// If the file is big-endian, this is represented as the bytes 0x04 0x03 0x02 0x01. When reading endianness as a 32-bit integer produces the value 0x04030201,
        /// the endianness of the file matches the endianness of the program that is reading the file, and no conversion is necessary.
        /// When reading endianness as a 32-bit integer produces the value 0x01020304, the endianness of the file is opposite to the endianness of the program that is reading the file.
        /// In that case, the program reading the file must convert all header bytes to the endianness of the program (i.e., a little-endian program must convert from big-endian,
        ...
        /// this should be the size in bytes corresponding to glType. For texture data that does not depend on platform endianness, including compressed texture data, glTypeSize must equal 1.
        /// </summary>
        ...
        /// <summary>
        /// For compressed textures, glInternalFormat must match the compressed internal format, usually one of the values from table 8.14 of the OpenGL 4.4 specification [OPENGL44].
        /// For uncompressed textures, glInternalFormat specifies the internal format parameter passed to glTexStorage*D or glTexImage*D, usually one of the sized internal formats from
        /// tables 8.12 and 8.13 of the OpenGL 4.4 specification [OPENGL44]. The sized format should be chosen to match the bit depth of the data provided. glInternalFormat is used when
        ...
        /// the internal format parameter is required to have the same value as the format parameter.
        /// </summary>
        ...
        /// OpenGL 4.4 specification [OPENGL44] (RGB, RGBA, ALPHA, etc.). For uncompressed textures, this value will be the same as glFormat and is used as the internal format parameter
        ...
        /// <summary>
        /// numberOfArrayElements specifies the number of array elements. If the texture is not an array texture, numberOfArrayElements must equal zero.
        /// </summary>
        ...
        /// <summary>
        /// numberOfMipmapLevels must be 1 for non-mipmapped textures. For mipmapped textures, it equals the number of mipmaps. Mipmaps are stored in order from largest size to smallest
